...
-(void) tryFindPathToKobold2D
{
	...
	{
		debugging = YES;
		[newProjectName setBackgroundColor:[NSColor cyanColor]];
		[self tryFindPathToKobold2D];
	}
}
...
-(void) createProjectWithName:(NSString*)projectName fromTemplate:(NSString*)templateName intoWorkspace:(NSString*)workspaceName
{
	...
	{
		NSString* msg = [NSString stringWithFormat:@"A project with the name '%@' already exists!", templateName];
...
-(IBAction) createProject:(id)sender 
{
	...
	{
		NSString* projectName = [newProjectName stringValue];
