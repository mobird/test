...
-(id) init
{
	if ((self = [super init]))
	{
		...
		//tileMap.position = CGPointMake(0, -240);

		// hide the event layer, we only need this information for code, not to display it
		CCTMXLayer* eventLayer = [tileMap layerNamed:@"b1"];
		eventLayer.visible = YES;

		CCTMXLayer* winterLayer = [tileMap layerNamed:@"b2"];
...
//	CCTMXObjectGroup* objectLayer = [tileMap objectGroupNamed:@"sprites"];
//	NSAssert([objectLayer isKindOfClass:[CCTMXObjectGroup class]], @"ObjectLayer not found or not a CCTMXObjectGroup");
//	
